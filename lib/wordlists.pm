...
sub get {
    ...
    if ( $wordlist_file ne "new" ) {
        open TEXT, "<", "$res/lists/$wordlist_file.txt"
          or die "Can't open $wordlist_file: $!";
    }
    else {
        print "\e[2J\e[H";    # Clear screen and move cursor to top-left corner
        ...
        open TEXT, "<", "$res/lists/$wordlist_file.txt"
          or die "Can't open res/lists/$wordlist_file.txt: $!";
    }
    @words = <TEXT>;
    close TEXT;
    chomp @words;
...
sub words_creation {
    ...
    open( my $fh, '<', "$res/1000-most-common-words.txt" )
      or die "Could not open file 'res/1000-most-common-words.txt' $!";
    my @words = <$fh>;
    close $fh;
    ...
    open( $fh, '>', "$res/lists/tmp.txt" )
      or die "Could not open file 'res/lists/new.txt' $!";
    print $fh $wordlist;
    close $fh;
    return "tmp";
}
...
sub random_creation {
    ...
    for ( my $size = 1 ; $size <= $avg_word_size ; $size++ ) {
        my $probability =
          exp( -( ( $size - $avg_word_size )**2 ) / ( 2 * $sigma**2 ) );
    ...
    open( my $fh, '>', "$res/lists/$file_name.txt" )
      or die "Could not open file 'res/lists/$file_name.txt' $!";
