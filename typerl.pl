...
sub start {
    ...
    print "*" x 80 . "\n";
    print
"*              Press 'esc' anytime to exit and 'tab' to restart.               *\n";
    print "*" x 80 . "\n";
    ...
    for ( my $n = 0 ; $n < scalar(@lines) ; $n++ ) {
        ...
        for ( my $i = 0 ; $i < length($line) ; $i++ ) {
            ...
            $char_input = Term::ReadKey::ReadKey(0);
            ...
            elsif ( $char_input eq "\t" and $total_chars > 0 ) {
                metronome::stop($metronome_pid);
                start();
            }
    ...
    printf "│%-5s│%-4s│%-5s│%-6s│%-8s│\n",
      ...
      sprintf( "%.0f", $cpm ), sprintf( "%.0f", $wpm ), $errors,
      sprintf( "%.2f", $accuracy ) . "%";
    printf
"├─────┴────┴─────┴──────┴────────┴─────────────────────────────────────────────┐\n";
    ...
    foreach my $char ( sort keys %char_times ) {
        my $char_avg_time =
          sprintf( "%.2f", average( @{ $char_times{$char} } ) );
    ...
    printf "│%-5s│%-4s│%-7s│%-5s│█",  "Char", "CPM", "Correct", "% Acc";
    printf "│%-5s│%-4s│%-7s│%-5s│█",  "Char", "CPM", "Correct", "% Acc";
    ...
    foreach my $char ( sort keys %char_times ) {
        ...
        my $char_avg_time =
          sprintf( "%.2f", average( @{ $char_times{$char} } ) );
        my $char_accuracy =
          sprintf( "%.2f", 100 * ( 1 - ( $incorrect_chars{$char} / $total ) ) );
        ...
        my $color =
          get_color( $cpm, $min_cpm, $max_cpm, $char_cpm, $char_accuracy );
        print $color;
        if ( $layout_name ne "none" ) {
...
sub export {
    ...
    if ( -e $export_file ) {
        open( my $fh, '>>', $export_file )
          or die "Could not open file '$export_file' $!";
        print $fh $data;
        close $fh;
    }
    else {
...
sub get_color {
    ...
    $green_percent =
        ( $green_percent < 0 )   ? 0
      : ( $green_percent > 100 ) ? 255
      :                            int( $green_percent * 2.55 );
    $red_percent =
        ( $red_percent < 0 )   ? 0
      : ( $red_percent > 100 ) ? 255
      :                          int( $red_percent * 2.55 );
